...
const getCurrentLang = () => (rootElement.getAttribute('lang') === 'en' ? 'en' : 'es');
const getCurrentTheme = () => rootElement.getAttribute('data-theme') === 'light' ? 'light' : 'dark';
...
const scrollAnimatedSections = Array.from(document.querySelectorAll('main.container.hero-grid, section.container, footer.container.site-footer'));
const canUseSectionAnimations = window.matchMedia('(min-width: 861px) and (prefers-reduced-motion: no-preference)').matches;
...
if (contactForm instanceof HTMLFormElement) {
    ...
    contactForm.addEventListener('submit', async (event) => {
        ...
        const previousLabel = submitButton instanceof HTMLButtonElement ? submitButton.textContent : '';
        ...
        try {
            ...
            if (response.ok) {
                ...
                const apiError = payload && Array.isArray(payload.errors) && payload.errors.length > 0
...
if (lazyImages.length > 0) {
    ...
    const lowPowerDevice = (typeof nav.deviceMemory === 'number' && nav.deviceMemory <= 4) ||
    ...
    const getImageSectionId = (img) => {
        ...
        if (section.matches('main.container.hero-grid'))
            return 'top';
        if (section.matches('footer.container.site-footer'))
            return 'contacto';
        return 'default';
    };
    ...
    if ('IntersectionObserver' in window) {
        ...
        const getObserverForMargin = (marginPx) => {
            if (!observerByMargin.has(marginPx)) {
                observerByMargin.set(marginPx, new IntersectionObserver(handleImageWarmup, {
...
(function () {
    ...
    const renderHobbiesWithReact = (hobbiesGrid, hobbies) => {
        ...
        const cards = hobbies.map((hobby, index) => h('article', {
            className: `hobby-card card ${hobby.cardClass || ''}`,
            key: `${hobby.title}-${index}`,
        }, h('span', {
            className: `hobby-icon ${hobby.iconClass || ''}`,
            'aria-hidden': 'true',
        }, h('i', { className: hobby.icon })), h('h4', { dangerouslySetInnerHTML: html(hobby.title) }), h('p', {
        ...
        const createRoot = ReactDOMRef.createRoot || (ReactDOMRef.default && ReactDOMRef.default.createRoot);
